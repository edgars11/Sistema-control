...
function guardarMovimiento(e) {
    ...
    if (pago_id == 5 && (numIdClient == '9999999999999' || idCamalClient == idCliente)) {
        swal.fire({
            title: "Error Cliente",
...
$(document).ready(function () {
    ...
    $('#table_data').DataTable({
        ...
        "ajax": {
            ...
            type: "post",
            data: { salida_fecha: dateSalida, suc_id: suc_idx, salida_tipo: 'PF'}
        },
        "bDestroy": true,
        "responsive": true,
        "bInfo": true,
...
function setCount() {
    let fecha = $('#sal_fecha').val();
    $.post("../../controllers/salidaLoteController.php?op=AmountCamal", { fecha_registro: fecha }, function (data) {
        if (data.length > 0) {
            data = JSON.parse(data);
            if (data.cantidadCamal == null) {
                $('#lote_cant_act').val('0');
                swal.fire({
                    title: "Error consulta",
...
function selectCliente(cli_id) {
    ...
    if (idCamalClient == cli_id) {
        swal.fire({
            title: "Error consulta",
            text: "El cliente seleccionado no es válido para esta operación!",
            icon: "warning"
        });
        return;
    }
