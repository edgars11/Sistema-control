...
class SalidaLote extends Conectar
{
    ...
    public function insertarSalidaLote(
        ...
        $conectar = parent::Conexion();
        $sql = "exec sp_crud_salida_lote @i_operacion=?,@i_salida_fecha=?, @i_lote_id=?, @i_salida_cantidad=?, 	@i_salida_peso=?, @i_salida_tara=?, @i_salida_peso_neto=?, 	@i_salida_precio=?,	@i_salida_total=?, @i_salida_tipo=?, @i_cli_id=?, @i_usu_id=?, @i_suc_id=?";
        $query = $conectar->prepare($sql);
        $query->bindValue(1, 'C');
